...
- **`NeedsInput`** — the step is an interactive turn paused ready for the operator's next message. It is *not* awaiting approval; an ordinary chat turn is this kind, and must not demand a review decision. Only interactive-session steps (§16 / the daemon's session materializer — a real, specified component per room spec §5; this passing reference was the honest side of the §20 contradiction) declare it.
