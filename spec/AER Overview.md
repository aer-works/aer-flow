...
| **UI** (and any other client) | `aer-flow-ui-behavioral-spec-v1.0.md` | Visualizing pipeline state and authoring workflow templates. Has no execution authority of its own — every mutation it makes goes through the same interface any other client would use. Fully replaceable; a terminal, a desktop app, and an IDE plugin are all equally valid implementations.                                                           |
