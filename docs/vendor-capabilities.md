...
| Always-fires gate hook | **`PreToolUse` hook — first of six evaluation steps, applies even in `bypassPermissions`** | **`PreToolUse` hook — the vendor mechanism denies; what AER shipped did not run at all until #710** |
...
AER's gate, and reading it as the latter is how #710 stayed invisible.** What AER actually shipped did
not run at all; the measurement and its consequences are in
