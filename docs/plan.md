...
| [0046](decisions/0046-a-room-is-a-container.md) | **A room is a container; a workflow is work run under it that keeps its name while running** — amends 0001's room/workflow boundary: the room is the durable place (directory, memory, participants, orchestrator), a workflow is a unit of work run under it, and delegation nests *workflow runs*, not rooms, with deliberate promotion to a child room. Because 0009's retention is unbuilt, the container model creates the requirement for it, scoped as #903 (a #778 prerequisite). |
| [0047](decisions/0047-workflow-templates-are-data-over-roles.md) | **Workflow templates are data composed over the role catalog; a composer builds the DAG with a closed engine-defined capture-step operation set** — the rung-3 architecture: a template is a named, ordered list of role-phases plus declared inputs, resolved like the role catalog and dispatched via `RoleDispatch`; the capture step (e.g. `diff-of-work-so-far`) is a closed, engine-defined operation, never a template-supplied command, grounded in the engine-does-no-permission-check reality. Folds #901. |
| [0048](decisions/0048-oversized-worker-input-travels-in-a-file.md) | **Oversized worker input travels in a file the worker reads, not a bigger command line** — the #932 argv-ceiling gap is answered by file-passing (the review-lane pattern #789, vendor-symmetric, no ABI change), not by an stdin/input-bytes channel on the spawn ABI (claude-only, measured #934; largely redundant with file-passing). The ceiling guard stays and its refusal now teaches file-passing. Revisit the ABI only on a concrete #778 need. |
| [0049](decisions/0049-the-wake-loop-is-in-contract-and-the-orchestrator-decides.md) | **The wake loop is in-contract, and the room orchestrator is a resident that makes decisions** — `RoomWakeBridge` is recorded as what the engine spec fenced off and foresaw: a separate watch-and-react layer, a client of the engine (derived wakes are never authority; its writes are attributed journal events). A room's orchestrator exists always — the dial is authority, with escalate-everything demoted to a build-sequencing milestone — and its authority model mirrors the vendor-CLI permission experience while AER enforces grants at the daemon tool boundary for every vendor (the record's Harder notes carry the per-vendor mechanics, e.g. agy's room-scoping rides session identity, not tool exposure). Owed: signed actions, a concurrency/rate leash, producer≠decider, escalation as an outcome; #594/#802 vendor-readiness moves onto the critical path. Narrows 0038. |
