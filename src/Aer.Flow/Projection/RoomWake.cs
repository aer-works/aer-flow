...
/// orchestrator turn (#799). <see cref="DispatchedLaneTerminated"/> and
/// <see cref="EscalatedLaneTerminated"/> are the design comment's single "unresolved ref whose
/// lane shows a terminal event" clause, split by <see cref="HeldWorkStatus"/> because the two are
...
/// reason. <see cref="DispatchOrphaned"/> is the #774 pattern: dispatch recorded, lane journal
/// never appeared.
/// </summary>
public enum RoomWakeKind
{
    DispatchedLaneTerminated,
    EscalatedLaneTerminated,
    DispatchOrphaned,
}
...
/// <summary>
/// The read-only probe of a single lane directory the derivation needs: whether its
...
/// </summary>
public readonly record struct LaneProbeResult(bool JournalExists, bool IsTerminal);

/// <summary>
/// Wake set = f(<see cref="RoomState"/>, lane probes) — pure, synchronous, no wall-clock, no I/O.
...
{
    /// <summary>
    /// Derives every wake in <paramref name="roomState"/> given <paramref name="laneProbes"/> — a
    /// probe result per <see cref="HeldWorkRef"/> the caller chose to watch. A ref with no entry
    /// in <paramref name="laneProbes"/> is treated as not (yet) probed and produces no wake, never
    ...
    public static IReadOnlyList<RoomWake> DeriveWakes(
        RoomState roomState,
        IReadOnlyDictionary<HeldWorkRef, LaneProbeResult> laneProbes)
    {
        ArgumentNullException.ThrowIfNull(roomState);
        ArgumentNullException.ThrowIfNull(laneProbes);
        ...
        foreach (var (@ref, state) in roomState.HeldWork)
        {
            ...
            // regardless of what its lane journal now shows.
            if (state.Status == HeldWorkStatus.Resolved)
            {
                continue;
            }

            // A memory proposal's ref is a capture file, not a lane directory (#801) — every
            // lane-probe-derived wake kind is meaningless for it: "no journal" is its normal
            // pending state, not an orphaned dispatch (#832). Waiting on an operator decision is
            ...
            if (probe.IsTerminal)
            {
                wakes.Add(new RoomWake(
                    ...
                    state.Status == HeldWorkStatus.Escalated
                        ? RoomWakeKind.EscalatedLaneTerminated
                        : RoomWakeKind.DispatchedLaneTerminated));
            }
        }
