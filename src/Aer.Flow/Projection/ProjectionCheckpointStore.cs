...
public static class ProjectionCheckpointStore
{
    ...
    public static ProjectionCheckpoint? Load(string taskDirectoryPath)
    {
        ...
        try
        {
            ...
            if (checkpoint is null || checkpoint.EventOffset < 0 || checkpoint.State is null)
            {
                Console.Error.WriteLine($"[ProjectionCheckpoint] Fallback to full replay LOUDLY: Checkpoint file '{filePath}' deserialized to null or negative offset.");
                return null;
            }
