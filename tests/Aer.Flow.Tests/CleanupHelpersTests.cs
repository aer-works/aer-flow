namespace Aer.Flow.Tests;

/// <summary>
/// Proves the <see cref="Aer.Tests.Shared.FileCleanup"/> helpers (and the shared
/// <c>CleanupRetry</c> core behind them and <see cref="Aer.Tests.Shared.DirectoryCleanup"/>) do what
/// the #918 sweep relies on: retry past a transient Windows share-lock, and split on a persistent one
/// — teardown (<see cref="Aer.Tests.Shared.FileCleanup.Delete"/>) swallows it so it can't mask a test
/// result, setup (<see cref="Aer.Tests.Shared.FileCleanup.EnsureDeleted"/>) surfaces it.
/// </summary>
