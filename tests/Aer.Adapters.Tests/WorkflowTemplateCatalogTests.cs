...
/// templates, unknown role references, and invalid input declarations.
/// </summary>
...
public class WorkflowTemplateCatalogTests
{
    ...
    private static string Phase(string roleId = "implement", string instruction = "instr", bool askFirst = false) =>
        $$"""{"role_id":"{{roleId}}","instruction":"{{instruction}}","ask_first":{{(askFirst ? "true" : "false")}} }""";

    private static string Template(string id, string phases, string inputs = """["diff-of-work-so-far"]""") =>
        $$"""{"id":"{{id}}","phases":{{phases}},"inputs":{{inputs}}}""";
    ...
    public void The_env_override_takes_precedence()
    {
        ...
        using var env = PointAt(
            cat,
            $"[{Template("custom-tpl", $"[{Phase("implement", "custom instr")}]")}]");
    ...
    [Fact]
    public void A_template_declaring_an_unknown_input_throws()
    {
        using var cat = new TempCatalog();
        using var env = PointAt(
            cat,
    ...
    [Fact]
    public void A_template_missing_a_required_field_fails_loudly()
    {
        using var cat = new TempCatalog();
        using var env = PointAt(
            cat,
            """[{"id":"t","phases":[{"role_id":"implement","instruction":"i"}]}]""");
