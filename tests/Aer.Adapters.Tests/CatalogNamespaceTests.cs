...
/// never both, so <c>aer dispatch &lt;name&gt;</c> is unambiguous. <c>DispatchCommand</c> (in Aer.Cli)
...
public sealed class CatalogNamespaceTests : IDisposable
{
    ...
    public void No_shipped_template_id_collides_with_a_shipped_role_id()
    {
        ...
        Assert.True(
            ...
            + "Dispatch is one namespace (0047 §5) — rename one so 'aer dispatch <name>' stays unambiguous.");
    }
}
