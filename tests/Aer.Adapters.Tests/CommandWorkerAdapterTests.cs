...
public class CommandWorkerAdapterTests
{
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "aer_cmd_test_" + Guid.NewGuid().ToString("N"));
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "aer_cmd_fail_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempDir);
        var logPath = Path.Combine(tempDir, "flow.jsonl");
        try
        {
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "aer_cmd_missing_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempDir);
        var logPath = Path.Combine(tempDir, "flow.jsonl");
        try
        {
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "aer_cmd_gitrepo_" + Guid.NewGuid().ToString("N"));
