...
- [x] Phase 5 — Task & Session Lifecycle Management
...
- **Post-hoc correction (retroactive Phase 1 gap-fill, #262): minimal-overhead dispatch (Claude's `--bare`) is never used, contrary to this phase's original plan.** Live-verified against the real, authenticated `claude` CLI (2.1.217): `claude --help` documents `--bare` as skipping "keychain reads" — exactly where subscription OAuth login lives — and a `--bare` dispatch against a valid, unexpired subscription login fails immediately with "Not logged in · Please run /login". `InteractiveSessionMaterializer.Materialize` originally hardcoded `MinimalOverhead: true` for every session, meaning every interactive-session turn silently failed authentication against the primary supported auth path (subscriptions, not API keys — CLAUDE.md's own stated premise). Fixed by defaulting `MinimalOverhead: false`; no request-level override exists to opt back in, since doing so would only ever be safe for the API-key auth path this project doesn't manage keys for anyway.
- **Verification**: a multi-turn session confirms native session resumption (not re-sent context) on same-vendor turns; a stress test drives ~50-100 programmatic turns on one step to confirm the repeated-supersede pattern holds at real chat frequency (projection cost, artifact directory naming, replay time) — done via a stub adapter (pure engine mechanics, no vendor dependency) plus a live 50-turn smoke gate (`pixi run smoke-session`); a vendor-handoff test where turn 2 goes to a different adapter than turn 1 and still has turn 1's context — stub-covered for the branching logic, live handoff-retains-context still manual (see `docs/runbooks/live-session-smoke.md`); a latency comparison with/without minimal-overhead dispatch — moot now that minimal-overhead dispatch is never used; ceiling enforcement test — stub-covered; a Claude turn with at least one tool call shows partial messages/tool-call events arriving incrementally in the UI during the run, not only after exit; a Gemini turn confirms the generic running indicator appears and clears correctly with no fabricated detail.
...
- **Post-hoc correction (retroactive Phase 2 gap-fill, #263) — two gaps found after the phase was marked done, both closed:** (1) Compact didn't actually start a fresh session — see Phase 1's own post-hoc correction note; fixed via a `forceHandoff` parameter. (2) Discovery was built and tested at the API level (`GET /api/sessions/{id}/commands`), but the phase's actual goal — "let the user **see and invoke**" — was never finished: neither `Aer.Ui` nor `Aer.Mobile` ever called the endpoint. Fixed: a Commands picker in both apps' chat UI (recent-first, split into invokable command/skill/agent kinds vs. Gemini's informational-only mode/plugin kinds), plus a session-level mode endpoint (`POST /api/sessions/{id}/mode`, "auto"/"default"/"plan") since `PermissionGrant` already persisted across turns but had no way to be changed mid-session.
- **Open question, not yet scoped**: the owner wants to eventually explore a complete abstraction over aggregate/session-level commands across whatever workers are active in a session or multi-vendor dialogue — the session-level mode endpoint above is a first concrete step toward this. The harder example: something like `/context` shared continuously across every worker in a multi-vendor dialogue, rather than only synthesized once at a vendor handoff (`SynthesizeContextSummary`). Not designed or scoped here — revisit once session-level mode has real usage.
- **Verification**: discovery against this repo's own `.claude/` and `agy`'s real subcommands returns a non-empty, correctly-shaped list; a compact round-trip shrinks the stored transcript while preserving enough context that the next turn still makes sense (AER-side plumbing only — see Phase 1's note on what "shrinks" can and can't mean at this level).
...
- **Fleet-status list (folded in from design review, 2026-07-21)**: the archive/delete screen this phase already needs — a list of every non-archived task/session — becomes the first "all active work at once" view either app has ever had, if it shows live status instead of just names. Every current screen (desktop's Task view, mobile's Inbox) shows exactly one open task/session at a time, reinforced rather than relaxed by PR #276's per-client-viewing fix (each client filters to whichever single directory it has open); there is no endpoint today that reports status across multiple task directories at once (`GET /api/tasks/recent` returns bare path strings only). This phase adds a new daemon endpoint (e.g. `GET /api/tasks`) returning each known task/session directory's lightweight status (friendly name, workflow template id or "interactive session", top-level status, paused-step count) without fully loading every directory's projection, and both apps' lifecycle-management list renders it instead of a bare name list. Full per-workflow DAG/progress-bar dashboards (the "control room" idea from that same design review) stay out of scope here — this is the list, not the dashboard.
- **Verification**: archiving a task/session removes it from the default list view (desktop and mobile) but it is still directly loadable and reappears once unarchived; deleting removes the directory and it no longer appears in any listing; a regression test confirms a new task/session can reuse a name freed by deleting the earlier one; no change to how an unarchived, non-deleted task/session behaves today; the lifecycle list shows each active task/session's current status (not just its name) on both desktop and mobile, sourced from the new endpoint rather than a per-item full projection load.
- **Review follow-up: `DirectoryPath` containment guard.** Archive/unarchive/delete originally took a caller-supplied `DirectoryPath` straight into filesystem mutation (a real recursive `Directory.Delete` for delete) with no check that it actually named something under `~/.aer/tasks`/`~/.aer/sessions` — the same vulnerability class issue #250 fixed for `RunTemplate`'s `TaskName`, but worse: reachable from any paired remote client (mobile's `DaemonClient.deleteTask()` included) with no path-traversal trick needed at all, just any absolute path. Fixed by requiring the resolved path be contained within one of the two managed roots, mirroring #250's guard; covered by a new path-traversal regression test.
- **Manual verification, live**: the desktop Tasks view was exercised end to end against a real running daemon (not just automated tests) — navigated to the Tasks section, archived a real task directory, confirmed it dropped from the default list and reappeared under "Show archived" with the item correctly marked, unarchived it back, then ran the full Delete → "Really delete?" two-step confirm → Cancel (reverts cleanly) → Delete → "Yes, delete" (removes the directory from disk and the fleet list) flow. `flutter test` could not be run locally on this Windows host for the mobile side — the `tailscale` package's native-asset build hook requires a C compiler on `PATH` for its Go cgo cross-compile, which this machine lacks; CI's `ubuntu-latest` job has the Go toolchain this needs. `flutter analyze` passed clean locally.
- **Also re-verified as part of this PR**: Phase 1's `--bare`/subscription-auth fix (see Phase 1's own post-hoc correction note) — `pixi run smoke-session` was re-run twice via a real authenticated `claude` CLI after that fix and passed both times (50 sequential turns held, compact produced a real, substantial vendor summary), closing the gap where that fix had gone unverified live.
